...
fees = {
    'binance': {'base': 0, 'quote': 0},
    'okx': {'base': 0.0005, 'quote': 0.0005},
    'bitget': {'base': 0.001, 'quote': 0.001},
    'bybit': {'base': 0.001, 'quote': 0.001},
    'gate': {'base': 0.0001, 'quote': 0.0001},
    'kucoin': {'base': 0.0008, 'quote': 0.0008},
}
...
criteria_pct = 0.1  # Lower percentage threshold to capture smaller spreads
criteria_usd = 1    # Minimum profit in USD required
...
def send_to_telegram(message):
    ...
    try:
        if telegram_sending:
            # put this if you want to send telegram messages.
...
def printerror(**args):
    if 'm' in list(args.keys()):
        print(
            f"{get_time()}{Fore.RED}{Style.BRIGHT}Error: {args['m']}{Style.RESET_ALL}")
        append_new_line("logs/logs.txt",
                        f"{get_time_blank()} ERROR: {args['m']}")
    else:
    ...
    if 'name_of_data' in list(args.keys()) and 'data' in list(args.keys()) and 'm' in list(args.keys()):
        append_new_line(
            "logs/logs.txt", f"{get_time_blank()} ERROR: {args['m']} | {args['name_of_data']}: {args['data']}")

    elif 'name_of_data' in list(args.keys()) and 'data' in list(args.keys()):
        append_new_line(
            "logs/logs.txt", f"{get_time_blank()} ERROR | {args['name_of_data']}: {args['data']}")
    ...
    for echange in exlist:
        try:
            if ex[echange].has['cancelAllOrders'] and ex[echange].fetchOpenOrders(pair_to_sell) != []:
                ...
                append_new_line(
                    'logs/logs.txt', f"{get_time_blank()} INFO: successfully canceled all orders on {echange}.")
            bal = get_balance(echange, pair_to_sell)
            m = ex[echange].load_markets()
            t = ex[echange].fetch_ticker(pair_to_sell)
            m[pair_to_sell]['limits']['cost']['min'] = 0 if type(m[pair_to_sell]['limits']['cost']['min']) != float and type(
                m[pair_to_sell]['limits']['cost']['min']) != int else m[pair_to_sell]['limits']['cost']['min']
            m[pair_to_sell]['limits']['amount']['min'] = 0 if type(m[pair_to_sell]['limits']['amount']['min']) != float and type(
                m[pair_to_sell]['limits']['amount']['min']) != int else m[pair_to_sell]['limits']['amount']['min']
            m[pair_to_sell]['limits']['cost']['max'] = 10e13 if type(m[pair_to_sell]['limits']['cost']['max']) != float and type(
                m[pair_to_sell]['limits']['cost']['max']) != int else m[pair_to_sell]['limits']['cost']['max']
            m[pair_to_sell]['limits']['amount']['max'] = 10e13 if type(m[pair_to_sell]['limits']['amount']['max']) != float and type(
                m[pair_to_sell]['limits']['amount']['max']) != int else m[pair_to_sell]['limits']['amount']['max']
            if (bal > (m[pair_to_sell]['limits']['cost']['min']/float(t['last'])) and bal > m[pair_to_sell]['limits']['amount']['min']) and (bal < (m[pair_to_sell]['limits']['cost']['max']/float(t['last'])) and bal < m[pair_to_sell]['limits']['amount']['max']):
                ...
                print(
                    f"{get_time()} Successfully sold {bal} {pair_to_sell[:len(pair_to_sell)-5]} on {echange}.")
                append_new_line(
                    'logs/logs.txt', f"{get_time_blank()} INFO: Successfully sold {bal} {pair_to_sell[:len(pair_to_sell)-5]} on {echange}.")
            else:
                print(
                    f"{get_time()} Not enough {pair_to_sell[:len(pair_to_sell)-5]} on {echange}.")
                append_new_line(
                    'logs/logs.txt', f"{get_time_blank()} INFO: Successfully sold {bal} {pair_to_sell[:len(pair_to_sell)-5]} on {echange}.")
            i += 1
        except Exception as e:
            printerror(
                m=f'{get_time()} Error while selling {pair_to_sell} on {echange}. Error: {e}',)
